...
// Weighted bands along one axis — the canonical "two-or-more color fields"
// layout, and the one that honors area ratios (the extension contrast).
...
// Mondrian-ish grid of color blocks.
...
function shapes(ctx, w, h, colors, rng) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const size = rng.range(base * 0.22, base * 0.5);
    // Keep each shape fully on-canvas: its centre is inset by the shape's
    // half-extent (size / 2) plus a small margin. All three primitives span
    // ±size/2 around the centre, so the same inset works for each.
    const half = size / 2 + margin;
    const cx = rng.range(half, w - half);
    const cy = rng.range(half, h - half);
