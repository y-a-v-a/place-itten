// lib/itten.js
// Johannes Itten's colour theory, distilled into data + palette generators.
//
//  - ITTEN_WHEEL: the 12-hue colour wheel (Farbkreis).
//  - A small seeded PRNG (xmur3 -> mulberry32) so a given seed string always
//    yields the same palette.
//  - One generator per each of Itten's 7 colour contrasts. Each returns
...
// ---------------------------------------------------------------------------
// Colour maths. We work in RGB and produce hex strings for the renderer.
// ---------------------------------------------------------------------------
...
// Perceptual grey of a colour (Rec. 601 luma).
function grayOf(rgb) {
...
const generators = {
  ...
  // 5. Simultaneous contrast: a vivid hue beside a neutral grey nudged toward
  // its complement (the grey then appears to shimmer with that complement).
  simultaneous(rng) {
    const i = rng.int(0, WHEEL_SIZE - 1);
    const vivid = ITTEN_WHEEL[i].hex;
    const compRgb = hexToRgb(ITTEN_WHEEL[complementIndex(i)].hex);
    const grey = rgbToHex(mix({ r: 128, g: 128, b: 128 }, compRgb, 0.12));
    return { colors: [vivid, grey], weights: [1, 2] };
  },
  ...
  // colours occupy proportionally smaller areas).
  extension(rng) {
    const i = rng.int(0, WHEEL_SIZE - 1);
...
// ---------------------------------------------------------------------------
// pickPalette: choose (or honour) a contrast and build its palette.
